...
class BaseTrainer:
    ...
    def save_state(self, stats):
        ...
        obj = {
            "model": self.model.state_dict(),    #mend权重参数
        ...
        if os.path.exists(self.save_path):
            bk_path = f"{self.save_path}.bk"   #bk=back意思就是之前的
            LOG.info(f"Moving old archive to {bk_path}")
            os.rename(self.save_path, bk_path)    #把前一次不带bk的文件命名成带bk的，也就是旧文件了

        torch.save(obj, self.save_path)     #储存的除了权重还有其它一些训练验证信息
    ...
    def run(self):
        ...
        self.epoches = round(float(self.config.max_iters) / (len(self.train_set) / self.config.batch_size))  #由最大train_step次数计算epoch次数
        if self.epoches < 1:
            self.epoches = 1
        self.global_iter = 0
        should_stop = False
        n_edits_batch = []
        for epoch in tqdm(range(self.epoches),desc='epoch',total=self.epoches,):
            if should_stop:
                break
            for i, batch in tqdm(enumerate(self.train_loader),desc='step',total=len(self.train_set)):
                self.global_iter += 1
                ...
                if not self.config.eval_only:
                    ...
                    if self.global_iter % self.config.log_interval == 0:
                        ...
                        averager.reset()
                        self.echo(self.global_iter, avg_info)   #最近log_interval的训练平均信息
