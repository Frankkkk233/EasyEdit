...
def compute_z(
    ...
    # Get model parameters    #lm:language model
    lm_w, ln_f = (
    ...
    input_tok = tok(
        ...
        return_tensors="pt",
        padding=True,
    ).to(f"cuda:{hparams.device}")    #多template+question的id，没有ans
    ...
    for i in range(len(rewriting_prompts)):
        ex_len = input_tok["attention_mask"][i].sum()   #template+question的个token个数
        rewriting_targets[i, ex_len - len(target_ids) : ex_len] = target_ids   #制作完整的template+question+ans(target)的id
    ...
    for it in range(hparams.v_num_grad_steps):  #为了用delta拟合出label的gd次数
        opt.zero_grad()
        ...
        with nethook.TraceDict(
            ...
            if kl_distr_init is None:   #不论迭代多少次一直是第一次迭代时的数值
        ...
        loss = torch.gather(  #log_prob:token概率表，替换维度为2，即词表index
            log_probs,
            2,
        ...
        nll_loss_each = -(loss * mask.to(loss.device)).sum(1) / target_ids.size(0)  #target_label维度均值
        nll_loss = nll_loss_each.mean()   #batch维度均值
