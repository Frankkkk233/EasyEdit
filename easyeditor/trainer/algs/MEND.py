...
class MEND(EditableModel):
    ...
    def edit(self, batch, condition=None, detach_history=False, return_factors=False, **kwargs):
        ...
        if self.config.shared:
            ...
            #param_idx为一函数，接受两个参数n,p返回shape_dict里shape为p的第n个元素
            transformed_factors = {  #transformed_factors获取mend使用梯度信息(p.__x__, p.__delta__)算出的伪梯度信息
                n: self.mend[str(tuple(self.get_shape(p)))](
                    p.__x__, p.__delta__, param_idx(n, p)
                )
                for n, p in _inner_params(      #_inner_params抽出选取层的名n和权重参数组p，做成字典
        ...
        if isinstance(self.model, transformers.GPT2LMHeadModel):
            ...
            targ = "ji"
        mean_grads = {
            n: torch.einsum(f"bi,bj->{targ}", x, delta)        #对上式mend算出的的伪梯度信息结果做爱因斯坦求和，得到伪梯度(mend算法)
        ...
        updates = {n: lr * g for lr, (n, g) in zip(self.edit_lrs, mean_grads.items())}    #伪梯度学习率放缩获得更新矩阵
        ...
        for n, p in edited_model.named_parameters():
            if n in pset:    #pset是待编辑层名字
                new_params.append(p + updates[n].to(p.dtype))     #使用基于伪梯度的更新矩阵更新权重,p是原始参数
            else:
                new_params.append(p)

        edited_model.update_params(new_params)     #模型更新权重
        ...
        return (
            MEND(
                edited_model,       #更新完的模型;注:此时只是用伪梯度更新的模型,更新的好不好不知道,需要在之后作gd,使得mend模型更好利用伪梯度,这样使用伪梯度更新效果才会好
                self.config,
                self.model_constructor,
                self.mend,          #mend模型
                edit_lrs=self.edit_lrs, 
            ),   #这里是打包好信息发回去的，MEND本身作forward就是传入的llm的forward(这里相当于edited_model)，这里相当于是新实例化了一个MEND
            info_dict,
        )
