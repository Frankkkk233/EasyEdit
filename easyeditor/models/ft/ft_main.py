...
def execute_ft(
    ...
    **kwargs: Any,
    ...
    weights={}
    for rewrite_module_tmp in hparams.rewrite_module_tmp:
        weights.update({
            n: p
            for n, p in model.named_parameters()
            for layer in hparams.layers
            if rewrite_module_tmp.format(layer) in n
        })
    ...
    for it in range(hparams.num_steps):
        ...
        for txt, tgt in zip(
            ...
            if hparams.objective_optimization == 'prompt_last':
                last_token_inds = inputs["attention_mask"].sum(dim=1) - 1   #最后一个token的id
