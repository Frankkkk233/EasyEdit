...
class BaseEditor:
    ...
    def edit(self,
        ...
        if "requests" in kwargs.keys():
            ...
            if self.hparams.use_customized and self.hparams.alg_name=='DeepEdit-Api':
                kwargs.update({'alg_name':self.alg_name,
                           'use_customized':self.hparams.use_customized})
    ...
    def edit_requests(self,
        ...
        if 'pre_edit' in kwargs and kwargs['pre_edit'] is not None:
            ...
            for i, request in enumerate(tqdm(requests,desc='pre',total=len(requests))):
        ...
        def edit_func(request):
            ...
            if self.alg_name=='DeepEdit-Api':
                edit_info = self.apply_algo(
                    [request],
                    self.hparams,
                    **kwargs)
                return edit_info
        ...
        if sequential_edit:
            ...
            for i, request in enumerate(tqdm(requests,desc='post', total=len(requests))):
                edited_model, weights_copy, icl_examples = edit_func(request)
                edit_evaluation(all_metrics, request, edited_model, i, test_generation, icl_examples, **kwargs)
                if self.alg_name == 'KN' or self.alg_name == 'GRACE' or self.alg_name == 'WISE':
                    with torch.no_grad():
