...
class EditTrainer(BaseTrainer):
    ...
    def edit_step(self, batch, training: bool):
        ...
        start = time.time()
        if "cond" in batch:
            edited_model, model_info = self.model.edit(batch["edit_inner"], batch["cond"])   #这里的edited_model是mend调后的
        else:
            ...
            if 'loc' in batch.keys():
                ...
                if kl_mask.size(1) != base_logits.size(1):
                    if self.config.use_customized:
                        base_logits = base_logits[:, -kl_mask.size(1)-1:-1]
                        post_base_logits = post_base_logits[:, -kl_mask.size(1)-1:-1]
                    else:
        ...
        if training:
            ...
            )#这里回传更新mend_model的梯度，由于是training阶段，这里指标计算和test不一样(主要是为train服务的)，首先不需要看rephrase，其次loc的acc并没有计算
            #下面的指标对test来说没有参考

        # Collect some useful metrics
        with torch.no_grad():    #这里edit和loc调用的函数稍微有点不一样，edit计算acc的时候，调用的是带自定义参数值的masked_log_probs
        ...
        if 'loc' in batch.keys():
            ...
            info_dict["loss/loc"] = 0.1

        info_dict["edit/acc"] = post_edit_dict["acc"].item()     #编辑后reliability
        info_dict["edit/log_prob"] = post_edit_dict["log_prob"].item()
        info_dict["edit/prob"] = post_edit_dict["prob"].item()
        if 'loc' in batch.keys():
            info_dict["acc/pre"] = pre_loc_dict["acc"].item()    #编辑前loc_ans的reliability
            info_dict["acc/post"] = post_loc_dict["acc"].item()    #编辑后loc_ans的reliability
            info_dict["nll/pre"] = pre_loc_dict["nll"].item()     #这里注意一下命名，虽然没写loc，但要知道表示的是loc
            ...
            info_dict["acc/pre"] = 0.1
            info_dict["acc/post"] = 0.1
            info_dict["nll/pre"] = 0.1
            info_dict["nll/post"] = 0.1
            info_dict["n_tokens/pre"] = 0.1
            info_dict["n_tokens/post"] = 0.1
