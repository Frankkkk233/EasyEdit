...
def apply_deepedit_api_to_model(
    ...
    def call_llama(cur_prompt, stop, temperature=0):
        client = ZhipuAI(api_key = api_key)
        response = client.chat.completions.create(
            model="glm-4-plus",
            max_tokens=256,
    ...
    dataset = datasets
    new_facts = set()
    if kwargs['use_customized']:
        if kwargs['ds_name']=='cf':
            for d in dataset:
                target_new=d['target_new']
                d['target_new']={'str':target_new}
                d['prompt']=d['prompt'].replace(d['subject'], '{}')
                requested_rewrite=d.copy()
                d.update({'requested_rewrite':[requested_rewrite]})

        if kwargs['ds_name']=='mq':
            for d in dataset:
                target_new=d['target_new']
                d['target_new']={'str':target_new}
                d['prompt']=d['prompt'].replace(d['subject'], '{}')
                requested_rewrite=d.copy()
                d.update({'requested_rewrite':[requested_rewrite]})
                d.update({'questions':d['portability']['neighborhood']['prompt'],
                          'new_answer':d['portability']['neighborhood']['ground_truth'][0],
                          'new_answer_alias':[]})
